...
import { getExplainer } from "../../../../utilities/getExplainer";
...
export default function ExplainerView(props) {
    ...
    // Pickup the summary text using await from the getExplainer function and set it to the summary state once available
    useEffect(() => {
        const lookupSummary = async (link) => {
            let summaryLookup = await getExplainer(link);
        ...
        >
            <Modal.Header>
                ...
                <Modal
                    ...
                    // basic
                    content={
                        "This explanation was generated by AI from the page content on " +
                        displaySource +
                        ". See the Read view or Visit the site to see the full content."
                    }
                    header={"Where does this explainer come from?"}
                    trigger={
                        <div style={{ float: "right" }}>
                            <Button icon="info" size="tiny" circular basic compact/>
                            <Button
                                onClick={(e) =>
                                    handleClose(
                                        {
                ...
                <Modal.Description>
                    ...
                    >
                        <Dimmer active={summaryLoading} inverted>
                            <Loader inverted content="Generating explanation" />
                        </Dimmer>
                        ...
                        {summaryCollectionRequired ? (
                            ...
                            <Message warning>
                                Sorry, I wasn't able to generate an explanation
                                for this page. If you like, please wait a few
                        ...
                        handleOpenLink(
                            {
                                ...
                                event: "Engagement",
                                action: "Click",
                                channel: "Open Link from Explainer",
                                destination: domain,
                            },
                            e
                        )
