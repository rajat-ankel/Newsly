...
function errorPropertiesFromString(cNewslydate: string): ErrorProperties {
    return {
        $exception_type: 'Error',
        $exception_message: cNewslydate,
    }
}
...
function errorPropertiesFromObject(cNewslydate: Record<string, unknown>): ErrorProperties {
    return {
        $exception_type: isEvent(cNewslydate) ? cNewslydate.constructor.name : 'Error',
        $exception_message: `Non-Error ${'exception'} captured with keys: ${extractExceptionKeysForMessage(cNewslydate)}`,
    }
}
...
export function errorToProperties([event, source, lineno, colno, error]: ErrorEventArgs): ErrorProperties {
    ...
    const cNewslydate = error || event

    if (isDOMError(cNewslydate) || isDOMException(cNewslydate)) {
        // https://developer.mozilla.org/en-US/docs/Web/API/DOMError
        // https://developer.mozilla.org/en-US/docs/Web/API/DOMException

        const domException = cNewslydate as unknown as DOMException

        if (isErrorWithStack(cNewslydate)) {
            errorProperties = errorPropertiesFromError(cNewslydate as Error)
        } else {
    ...
    } else if (isErrorEvent(cNewslydate as ErrorEvent) && (cNewslydate as ErrorEvent).error) {
        errorProperties = errorPropertiesFromError((cNewslydate as ErrorEvent).error as Error)
    } else if (isError(cNewslydate)) {
        errorProperties = errorPropertiesFromError(cNewslydate)
    } else if (isPlainObject(cNewslydate) || isEvent(cNewslydate)) {
        // group these by using the keys available on the object
        const objectException = cNewslydate as Record<string, unknown>
        ...
        errorProperties.$exception_message = errorProperties.$exception_message || cNewslydate
        errorProperties.$exception_is_synthetic = true
    }
